...
class Permissions:
    @classmethod
    def can_view_paste(cls, paste_publicity: str, paste_owner_id: int, logged_in_user_id: int) -> bool:
        if paste_publicity == "listed" or paste_publicity == "unlisted":
            return True
    ...
    @classmethod
    def can_delete_chat_message(cls, token_level: str, paste_owner_id: int, logged_in_user_id: int) -> bool:
        # modify-level token is always required
        # if paste was not created anonymously, user is also required to be the owner
        return token_level == "modify" and (paste_owner_id is None or paste_owner_id == logged_in_user_id)
